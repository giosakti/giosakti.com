...
Since then, a wave of similar projects have appeared, each with a different focus. **IronClaw** (Rust, security-first with WASM sandboxing and capability-based permissions), **Nanobot** (Python, ~4K lines, very pragmatic), **NanoClaw** (minimal Claude SDK wrapper with a clever git-branch feature system), **Hermes Agent** (self-improving agents that create their own skills). I started building my own, Duragent, around the same time as most of these. We're all working on similar problems, making different trade-offs.
